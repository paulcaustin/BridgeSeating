...
int			flag;
{
	...
	for (player = 0; player < players * games; player++) {
		...
		if ((player % players) == players-1)
			putchar('\n');
		else if (player % players % PPERT == PPERT-1)
			putchar(' ');
	}
...
CHROM			*c;
{
	...
	for (game = 0; game < games; game++)
	for (i = 0; i < players-PPERT+1; i += PPERT) {
		// This only scores full tables
		p = &c->p[game * players + i];
		/*
		 * repeated play-with's
		 */
		notpresent = 0;
		for (p1 = 0; p1 < PPERT; p1++) {
			for (p2 = p1+1; p2 < PPERT; p2++) {
				old_score += TALLY(t, p[p1], p[p2]);
			}
			/*
			 * Special scoring for absent players
			 * playing with other absent players
			if (p[p1] >= PRESENT) notpresent++;
			 * Had to do with dinners, and not
			 * wanting more than one absent.
			 */
		}
		/* score += notpresent > 1 ? notpresent * 50 : 0; */

		/*
		 * repeated partnering, big no-no.
		 * Handles special positions within the table.
		 */
	...
	/*
	 * repeated tabling.
	 * If you want to avoid people sitting at the same
	 * table too many times.  This also counts people
	 * that are not seated at a table, i.e. extras.
	 */
	for (i = 0; i < players * games; i++) {
		p = &c->p[i];
		table = (i % players) / PPERT;
		score += BAD_TABLE * t3[p[0]*TABLES+table]++;
	}
...
CHROM			*c;
{
	...
	while (fgets(buf,BUFSIZ-1,in)) {
		...
		for (player = 0; i < len && player < players; i++) {
			...
			if (ch == '\t' || ch == ' ') continue;
			if      (ch >= 'A' && ch <= 'Z') ch += 0 - 'A';
			else if (ch >= 'a' && ch <= 'z') ch += 26 - 'a';
			else if (ch >= '0' && ch <= '9') ch += 52 - '0';
			else if (ch >= '#' && ch <= '$') ch += 62 - '#';
			else {
				fprintf(stderr, "Bad char: %x\n",ch);
				exit(1);
			}
			c->p[game*players+player++] = ch;
		}
		if (++game >= games)
			return(0);
	}
